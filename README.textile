...
You can optionally make an alias to use @ant-color@ instead of raw @ant@, and even reuse the default bash auto-completion for @ant-color@. To do so append the following line in your @~/.bashrc@:

bc. alias ant="ant-color"
complete -F _ant ant-color  # use the default Bash auto-completion of ant for ant-color
